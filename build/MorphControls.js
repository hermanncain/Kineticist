 ...
 * @author hermanncain / https://github.com/hermanncain
 */

THREE.MorphControls = function ( camera, domElement ) {
	...
	var _gizmo = new THREE.MorphControlsGizmo();
	this.add( _gizmo );

	var _plane = new THREE.MorphControlsPlane();
	this.add( _plane );
	...
	// updateMatrixWorld  updates key Morphation variables
	...
	this.pointerDown = function( pointer ) {
		...
		if ( ( pointer.button === 0 || pointer.button === undefined ) && this.axis !== null ) {
			...
			if ( planeIntersect ) {
				...
				let mtStart = new THREE.Vector3().fromArray(this.object.userData.morphTrans.bias);
				let mrStart = new THREE.Euler().fromArray(this.object.userData.morphTrans.rotation);
				let mqStart = new THREE.Quaternion().setFromEuler(mrStart);
				let msStart = new THREE.Vector3().fromArray(this.object.userData.morphTrans.size);
				_positionStart.copy( mtStart );
				_quaternionStart.copy( mqStart );
				_scaleStart.copy( msStart );
	...
	this.pointerMove = function( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			// Apply translate
			let mt = new THREE.Vector3().copy(pointEnd).sub( pointStart ).applyQuaternion( _quaternionStart );
			mt.add( _positionStart );
			if ( space === 'local' ) {
			...
			object.setMorphTranslation(mt.x,mt.y,mt.z);
			...
			// Apply morph scale
			let ms = new THREE.Vector3().copy(_scaleStart).multiply( _tempVector );
			object.setMorphScale(ms.x,ms.y,ms.z);
			...
			if ( space === 'local' ) {
				...
				let mq = new THREE.Quaternion().copy( _quaternionStart );
				mq.multiply( _tempQuaternion.setFromAxisAngle( rotationAxis, rotationAngle ) );
				let mr = new THREE.Euler().setFromQuaternion(mq);
				object.setMorphRotation(mr.x/Math.PI*180,mr.y/Math.PI*180,mr.z/Math.PI*180);
	...
	this.update = function () {
		...
		console.warn( 'THREE.MorphControls: update function has been depricated.' );

	};

};

THREE.MorphControls.prototype = Object.assign( Object.create( THREE.Object3D.prototype ), {

  constructor: THREE.MorphControls,

  isMorphControls: true

} );


THREE.MorphControlsGizmo = function () {

	'use strict';

	THREE.Object3D.call( this );

	this.type = 'MorphControlsGizmo';
	...
	matX.opacity = 0.7;
	matX.color.set( 0xdcdcdc );
	var matY = matX.clone();
	var matZ = matX.clone();

	var matInvisible = gizmoMaterial.clone();
	matInvisible.opacity = 0.6;

	var matHelper = gizmoMaterial.clone();
	matHelper.opacity = 0.6;
	...
	var cylinderGeometry = new THREE.CylinderBufferGeometry( 0.05, 0.05, 1, 12, 1, false);
	var torusGeometry = new THREE.TorusBufferGeometry(1,0.05,4,20);
	...
	// Special geometry for Morph helper. If scaled with position vector it spans from [0,0,0] to position
	...
	var gizmoTranslate = {
		X: [
			[ new THREE.Mesh( arrowGeometry, matX ), [ 1, 0, 0 ], [ 0, 0, -Math.PI / 2 ], null, 'fwd' ],
			[ new THREE.Mesh( arrowGeometry, matX ), [ 1, 0, 0 ], [ 0, 0, Math.PI / 2 ], null, 'bwd' ],
			[ new THREE.Mesh( cylinderGeometry, matX ), [ 0.5, 0, 0 ], [ 0, 0, Math.PI / 2 ]],
			// [ new THREE.Line( lineGeometry, matLineRed ) ]
		],
		Y: [
			[ new THREE.Mesh( arrowGeometry, matY ), [ 0, 1, 0 ], null, null, 'fwd' ],
			[ new THREE.Mesh( arrowGeometry, matY ), [ 0, 1, 0 ], [ Math.PI, 0, 0 ], null, 'bwd' ],
			[ new THREE.Mesh( cylinderGeometry, matY ), [ 0, 0.5, 0], [ Math.PI, 0, 0 ]],
			// [ new THREE.Line( lineGeometry, matLineGreen ), null, [ 0, 0, Math.PI / 2 ] ]
		],
		Z: [
			[ new THREE.Mesh( arrowGeometry, matZ ), [ 0, 0, 1 ], [ Math.PI / 2, 0, 0 ], null, 'fwd' ],
			[ new THREE.Mesh( arrowGeometry, matZ ), [ 0, 0, 1 ], [ -Math.PI / 2, 0, 0 ], null, 'bwd' ],
			[ new THREE.Mesh( cylinderGeometry, matZ ), [ 0, 0, 0.5 ], [ -Math.PI / 2, 0, 0 ]],
	...
	var gizmoRotate = {
		X: [
			[ new THREE.Mesh( new THREE.TorusBufferGeometry( 1, 0.05, 4, 24 ), matX ), [ 0, 0, 0 ], [ 0, -Math.PI / 2, -Math.PI / 2 ] ],
		],
		Y: [
			[ new THREE.Mesh( new THREE.TorusBufferGeometry( 1, 0.05, 4, 24 ), matY ), [ 0, 0, 0 ], [ Math.PI / 2, 0, 0 ] ],
		],
		Z: [
			[ new THREE.Mesh( new THREE.TorusBufferGeometry( 1, 0.05, 4, 24 ), matZ ), [ 0, 0, 0 ], [ 0, 0, -Math.PI / 2 ] ],
		],
		// X: [
		// 	[ new THREE.Mesh( torusGeometry, matRed ) ],
			// [ new THREE.Line( CircleGeometry( 1, 0.5 ), matLineRed ) ],
			// [ new THREE.Mesh( new THREE.OctahedronBufferGeometry( 0.04, 0 ), matRed ), [ 0, 0, 0.99 ], null, [ 1, 3, 1 ] ],
		// ],
		// Y: [
		// 	[ new THREE.Mesh( torusGeometry, matGreen ), null, [ 0, 0, -Math.PI / 2 ] ],

			// [ new THREE.Line( CircleGeometry( 1, 0.5 ), matLineGreen ), null, [ 0, 0, -Math.PI / 2 ] ],
			// [ new THREE.Mesh( new THREE.OctahedronBufferGeometry( 0.04, 0 ), matGreen ), [ 0, 0, 0.99 ], null, [ 3, 1, 1 ] ],
		// ],
		// Z: [
		// 	[ new THREE.Mesh( torusGeometry, matBlue ), null, [ 0, Math.PI / 2, 0 ] ],
	...
	var gizmoScale = {
		X: [
			[ new THREE.Mesh( new THREE.SphereBufferGeometry(0.1)/* scaleHandleGeometry */, matX ), [ 0.8, 0, 0 ], [ 0, 0, -Math.PI / 2 ] ],
			[ new THREE.Line( lineGeometry, matLineRed ), null, null, [ 0.8, 1, 1 ] ]
		],
		Y: [
			[ new THREE.Mesh( new THREE.SphereBufferGeometry(0.1)/* scaleHandleGeometry */, matY ), [ 0, 0.8, 0 ] ],
			[ new THREE.Line( lineGeometry, matLineGreen ), null, [ 0, 0, Math.PI / 2 ], [ 0.8, 1, 1 ] ]
		],
		// Z: [
		// 	[ new THREE.Mesh( new THREE.SphereBufferGeometry(0.1)/* scaleHandleGeometry */, matBlue ), [ 0, 0, 0.8 ], [ Math.PI / 2, 0, 0 ] ],
	...
	// updateMatrixWorld will update Morphations and appearance of individual handles
	...
	this.updateMatrixWorld = function () {
		...
		// Show only gizmos for current Morph mode
		...
		for ( var i = 0; i < handles.length; i++ ) {
			...
			if ( !this.enabled ) {
				...
				if ( handle.name === this.axis ) {
					handle.material.color.setHex(0xff8800);
					...
					handle.material.color.setHex(0xdcdcdc);
					// handle.material.opacity = 0.5;
					// handle.material.color.lerp( new THREE.Color( 1, 1, 1 ), 0.5 );
...
THREE.MorphControlsGizmo.prototype = Object.assign( Object.create( THREE.Object3D.prototype ), {

	constructor: THREE.MorphControlsGizmo,

	isMorphControlsGizmo: true

} );


THREE.MorphControlsPlane = function () {
	...
	this.updateMatrixWorld = function() {
		...
		// Align the plane for current Morph mode, axis and space.
...
THREE.MorphControlsPlane.prototype = Object.assign( Object.create( THREE.Mesh.prototype ), {

	constructor: THREE.MorphControlsPlane,

	isMorphControlsPlane: true
